function [wTotal,b,wcTotal] = afterChange_extractSurvey(data)
...
for k = 1:12
    ...
    wcTotal{k} = wc;
    b(1,k) = double(wc(1,k)/c); % b1 is prob of wc is shown in pos 1
    b(2,k) = double(wc(2,k)/c);
    b(3,k) = double(wc(3,k)/c);
