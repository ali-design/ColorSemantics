...
ix = find(cellfun(@(c)(isequal(c,'beach')),vocab(:)));
...
ix1 = find(cellfun(@(c)(isequal(c,'plants')),vocab(:)));
ix2 = find(cellfun(@(c)(isequal(c,'business')),vocab(:)));
ix3 = find(cellfun(@(c)(isequal(c,'interior')),vocab(:)));
...
% A = mixedColorTopicHist(find(mixedColorTopicHist));
% ix = find(mixedColorTopicHist);
...
%[costStruct,distanceCost] = LDA_512_imgMatching(p2,A,nMatch);
[costStruct,distanceCost] = LDA_512_imgMatching_parallel(p2,A,nMatch);
